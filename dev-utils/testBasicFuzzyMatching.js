...
function testBalancedMatching() {
  ...
  testCases.forEach(({ input, expectMatch, note }, index) => {
    ...
    if (matchDetails && teamId) {
      const status = '✅'
      console.log(
        `  ${status} Found: "${matchDetails.name}" (Score: ${matchDetails.score.toFixed(3)})`
      )
  ...
  console.log(
    `📊 Overall Results: ${successCount}/${testCases.length} tests passed (${((successCount / testCases.length) * 100).toFixed(1)}%)`
  )
  console.log(
    `📊 Originally Working: ${originallyWorkingSuccess}/${originallyWorkingCount} (${((originallyWorkingSuccess / originallyWorkingCount) * 100).toFixed(1)}%)`
  )
  console.log(
    `📊 Previously Problematic: ${problematicSuccess}/${problematicCount} (${((problematicSuccess / problematicCount) * 100).toFixed(1)}%)`
  )
...
function compareStrictMode() {
  ...
  testTeams.forEach(team => {
    ...
    console.log(
      `  Normal: ${normalMatch ? `"${normalMatch.name}" (${normalMatch.score.toFixed(3)})` : 'No match'}`
    )
    console.log(
      `  Strict: ${strictMatch ? `"${strictMatch.name}" (${strictMatch.score.toFixed(3)})` : 'No match'}`
    )
    console.log()
  })
}
...
function runAllTests() {
  ...
  console.log(
    `Overall success rate: ${((balancedResults.overall.passed / balancedResults.overall.total) * 100).toFixed(1)}%`
  )
  console.log(
    `Original functionality preserved: ${((balancedResults.original.passed / balancedResults.original.total) * 100).toFixed(1)}%`
  )
  console.log(
    `Problematic cases improved: ${((balancedResults.problematic.passed / balancedResults.problematic.total) * 100).toFixed(1)}%`
  )
}
...
export { testBalancedMatching, testCommonTeams, compareStrictMode, testFuzzyVsExact, runAllTests }

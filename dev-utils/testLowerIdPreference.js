...
function testLowerIdPreference() {
  ...
  testCases.forEach(teamName => {
    ...
    if (matchDetails) {
      console.log(
        `  Match: "${matchDetails.name}" (confidence: ${(matchDetails.confidence * 100).toFixed(1)}%)`
      )
    }
  })
}
...
export { testLowerIdPreference, demonstrateThresholdImpact, showTeamIdStatistics, usageExamples }

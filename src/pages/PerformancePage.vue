...
const calculateThresholds = () => {
    ...
    const allAvailableDivisions = [...new Set(allPlayersData.value.map(p => getPlayerDivision(p)).filter(Boolean))];
    ...
    const filteredByDivisionAndPosition = allPlayersData.value.filter(player => {
        ...
        const divisionMatch = selectedDivisions.value.length === 0 || 
                             selectedDivisions.value.length === allAvailableDivisions.length ||
                             selectedDivisions.value.includes(division);
        
        const positionMatch = selectedPositions.value.length === 0 || selectedPositions.value.some(selectedPos => {
            if (selectedPos === 'Goalkeeper') return player.shortPositions?.includes('GK');
            if (selectedPos === 'Defender') return player.shortPositions?.some(pos => ['DC', 'DR', 'DL', 'WBR', 'WBL'].includes(pos));
            if (selectedPos === 'Midfielder') return player.shortPositions?.some(pos => ['DM', 'MC', 'MR', 'ML', 'AMR', 'AMC', 'AML'].includes(pos));
    ...
    const playersAfterOverallFilter = filteredByDivisionAndPosition.filter(player => 
        (player.Overall || 0) >= overallThreshold
    );
...
const filteredPlayers = computed(() => {
    ...
    return allPlayersData.value.filter(player => {
        ...
        // Consider "all divisions" if empty array OR if all available divisions are selected
        const matchesDivision = selectedDivisions.value.length === 0 || 
                               selectedDivisions.value.length === allAvailableDivisions.length ||
                               selectedDivisions.value.includes(division);
        
        const matchesPosition = selectedPositions.value.length === 0 || selectedPositions.value.some(selectedPos => {
            if (selectedPos === 'Goalkeeper') return player.shortPositions?.includes('GK');
            if (selectedPos === 'Defender') return player.shortPositions?.some(pos => ['DC', 'DR', 'DL', 'WBR', 'WBL'].includes(pos));
            if (selectedPos === 'Midfielder') return player.shortPositions?.some(pos => ['DM', 'MC', 'MR', 'ML', 'AMR', 'AMC', 'AML'].includes(pos));
            if (selectedPos === 'Forward') return player.shortPositions?.includes('ST');
...
const scatterPlotConfigs = ref([
    ...
    { category: 'passing', group: 'progression', title: 'Passing Volume', xAxisKey: 'Ps C/90', yAxisKey: 'Pas %', xAxisLabel: 'Passes Completed per 90', yAxisLabel: 'Pass Completion %', quadrantLabels: { topRight: ['Elite Passer', ''], topLeft: ['Accurate Passer', ''], bottomRight: ['Volume Passer', ''], bottomLeft: ['Limited Passer', ''] }},
...
const fetchPlayersAndCurrency = async datasetId => {
    ...
    try {
        ...
        const allDivisions = [...new Set(allPlayersData.value.map(p => getPlayerDivision(p)).filter(Boolean))];
...
const filterPositionsFn = (val, update) => {
  update(() => {
    const needle = val.toLowerCase();
    positionOptions.value = availablePositions.value.filter(v => v.toLowerCase().indexOf(needle) > -1);
  })
};
...
watch(overallSliderValue, debounce((newValue) => {
    selectedOverall.value = newValue;
}, 300));

// Watch for changes that should recalculate thresholds
watch([selectedDivisions, selectedPositions], () => {
...
watch(availablePositions, (newPositions) => {

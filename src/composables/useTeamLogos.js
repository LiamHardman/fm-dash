...
export function useTeamLogos(options = {}) {
  ...
  const calculateSimilarity = (str1, str2) => {
    ...
    if (score >= 0.6) {
      ...
      if (maxLen <= 50) {
        // Only for reasonable length strings
  ...
  const isLikelyCorrectMatch = (searchName, foundName, score) => {
    ...
    const lengthRatio =
      Math.min(searchName.length, foundName.length) / Math.max(searchName.length, foundName.length)
  ...
  const getTeamId = teamName => {
    ...
    _bestScore = validCandidates[0].score

...
export function usePlayerFilters(props, emit) {
    ...
    const isFilterActive = (filterName) => {
        switch (filterName) {
            ...
            case 'transferValue':
                return filters.transferValue.min > props.transferValueRange?.min || 
                       filters.transferValue.max < props.transferValueRange?.max;
            case 'salary':
                return filters.salary.min > props.salaryRange?.min || 
                       filters.salary.max < props.salaryRange?.max;
            case 'clubs':
    ...
    const activeFilterCount = computed(() => {
        const filterNames = ['overall', 'potential', 'age', 'transferValue', 'salary', 'clubs', 'nationalities', 'positions', 'attributes'];

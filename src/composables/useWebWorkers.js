...
export function useWebWorkers() {
    ...
    const getPendingTaskCount = (workerName = null) => {
        if (workerName) {
            return Array.from(pendingTasks.value.values())
                .filter(task => task.workerName === workerName).length
        }
...
export function usePlayerCalculationWorker() {
    const { createWorker, executeTask, terminateWorker, getPendingTaskCount, hasWorker } = useWebWorkers()
            ...
            switch (operation.type) {
                case 'sort':
                    results[operation.id] = sortPlayersMainThread(
                        players, operation.fieldKey, operation.direction, 
                        operation.sortField, operation.isGoalkeeperView
                    )
                    break
                case 'filter':
